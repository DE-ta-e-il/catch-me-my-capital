...
source_tables = generate_table_names(Interval.QUARTERLY.label, STAT_NAME_LIST)

with DAG(
    dag_id="slv_economic_indicators_quarterly",
    description="Quarterly pipeline to transform economic indicators.",
    schedule_interval="0 0 15 */3 *",
    start_date=pendulum.datetime(2015, 1, 1),
    catchup=False,
    default_args=default_args,
    tags=[Layer.SILVER, Interval.QUARTERLY.label, FEATURE],
    template_searchpath=["/opt/airflow/dags/common/sql"],
) as dag:
    economic_indicators_sensors = [
        ExternalTaskSensor(
            task_id=f"wait_for_{stat_name.lower()}",
            external_dag_id="brz_economic_indicators_quarterly",
