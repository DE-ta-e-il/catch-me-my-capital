...
def get_govt_bond_data_kr(target, **ctxt):
    ...
    if FIRST_RUN:  # unravel the bunch
        ...
        for date, daily_list in gbd.items():
            upload = S3CreateObjectOperator(
                ...
                s3_bucket=S3_BUCKET,
                s3_key=f"bronze/govt_bonds_kr/kind={target}/date={date[:4]}-{date[5:7]}-{date[8:10]}/{target}_{date}.json",
    ...
    # Normal operation
        ...
        upload = S3CreateObjectOperator(
            ...
            aws_conn_id="aws_conn_id",
            s3_bucket=S3_BUCKET,
            s3_key=f"bronze/govt_bonds_kr/kind={target}/date={ds_year}-{ds_month}-{ds_day}/{target}_{ds_year}-{ds_month}-{ds_day}.json",
